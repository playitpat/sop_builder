...
def run(base=Path(".")):
    repo = ProjectRepository(base / "data" / "demo.db")
    pid = repo.create(TITLE)
    p = extract_process(TITLE, DESCRIPTION)
    initial = [g.label for g in governance_gaps(p)]
    apply_demo_clarification(p, CLARIFICATION)
    repo.save_process(pid, p, "ready")
    knowledge = LocalKnowledgeService(base / "knowledge_base").retrieve(
        TITLE + " " + DESCRIPTION
    )
    draft, reviews = review_loop(p)
    out = populate_template(
        base / "reference_documents/templates/TMP-10031_SOP_TemplateCC.docx",
        draft,
        base / "generated",
    )
